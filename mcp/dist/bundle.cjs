...
var require_ucs2length = __commonJS({
  "../../node_modules/ajv/lib/compile/ucs2length.js"(exports2, module2) {
    "use strict";
    module2.exports = function ucs2length(str2) {
...
// mcp/tools/issues/create.ts
var createIssueSchema = external_exports.object({
  title: external_exports.string().min(1).max(200).describe("Issue title"),
  description: external_exports.string().min(1).describe("Issue description or content"),
  type: external_exports.enum(["issue", "specification", "idea"]).optional().describe("Artifact type to create"),
  status: external_exports.enum(["open", "in_progress", "closed", "cancelled"]).optional().describe("Initial issue status"),
  priority: external_exports.enum(["low", "medium", "high", "critical"]).optional().describe("Issue priority level"),
  labels: external_exports.array(external_exports.string()).optional().describe("Labels to apply to the issue"),
  assignee: external_exports.string().optional().describe("User to assign the issue to"),
  project: external_exports.string().optional().describe("Project or epic to associate with"),
  wranglerContext: external_exports.object({
    agentId: external_exports.string().optional(),
    parentTaskId: external_exports.string().optional(),
    estimatedEffort: external_exports.string().optional()
  }).optional().describe("Wrangler-specific context information")
});
async function createIssueTool(params, providerFactory) {
  try {
...
// mcp/tools/issues/list.ts
var listIssuesSchema = external_exports.object({
  status: external_exports.array(external_exports.enum(["open", "in_progress", "closed", "cancelled"])).optional().describe("Filter by status"),
  priority: external_exports.array(external_exports.enum(["low", "medium", "high", "critical"])).optional().describe("Filter by priority"),
  labels: external_exports.array(external_exports.string()).optional().describe("Filter by labels"),
  assignee: external_exports.string().optional().describe("Filter by assignee"),
  project: external_exports.string().optional().describe("Filter by project"),
  parentTaskId: external_exports.string().optional().describe("Filter issues whose wranglerContext.parentTaskId matches"),
  types: external_exports.array(external_exports.enum(["issue", "specification", "idea"])).optional().describe("Filter by artifact types"),
  type: external_exports.enum(["issue", "specification", "idea"]).optional().describe("Filter by a single artifact type"),
  limit: external_exports.number().int().positive().max(1e3).optional().describe("Maximum number of issues to return"),
  offset: external_exports.number().int().min(0).optional().describe("Number of issues to skip for pagination")
});
async function listIssuesTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    const filters = {
...
function formatIssuesAsTable(issues) {
  ...
  const widths = headers.map(
    (header, i) => Math.max(header.length, Math.max(...rows.map((row) => (row[i] || "").toString().length)))
  );
  const separator = "| " + widths.map((w) => "-".repeat(w)).join(" | ") + " |";
  const headerRow = "| " + headers.map((h, i) => h.padEnd(widths[i])).join(" | ") + " |";
  const dataRows = rows.map(
    (row) => "| " + row.map((cell, i) => (cell || "").toString().padEnd(widths[i])).join(" | ") + " |"
  );
  return [headerRow, separator, ...dataRows].join("\n");
}

// mcp/types/issues.ts
var IssueArtifactTypeSchema = external_exports.enum(["issue", "specification", "idea"]);
var IssueStatusSchema = external_exports.enum(["open", "in_progress", "closed", "cancelled"]);
var IssuePrioritySchema = external_exports.enum(["low", "medium", "high", "critical"]);
var WranglerIssueContextSchema = external_exports.object({
  agentId: external_exports.string().optional(),
  parentTaskId: external_exports.string().optional(),
  estimatedEffort: external_exports.string().optional()
});
var IssueCreateRequestSchema = external_exports.object({
...
var IssueUpdateRequestSchema = external_exports.object({
...
var IssueFiltersSchema = external_exports.object({
  ids: external_exports.array(external_exports.string()).optional(),
  status: external_exports.array(IssueStatusSchema).optional(),
  priority: external_exports.array(IssuePrioritySchema).optional(),
  labels: external_exports.array(external_exports.string()).optional(),
  assignee: external_exports.string().optional(),
  project: external_exports.string().optional(),
  parentTaskId: external_exports.string().optional(),
  types: external_exports.array(IssueArtifactTypeSchema).optional(),
  type: IssueArtifactTypeSchema.optional(),
  createdAfter: external_exports.date().optional(),
  createdBefore: external_exports.date().optional(),
  limit: external_exports.number().int().positive().optional(),
  offset: external_exports.number().int().nonnegative().optional()
});
var IssueSearchOptionsSchema = external_exports.object({
  query: external_exports.string().min(1),
  fields: external_exports.array(external_exports.enum(["title", "description", "labels"])).optional(),
  filters: IssueFiltersSchema.optional(),
  sortBy: external_exports.enum(["created", "updated", "priority", "status"]).optional(),
  sortOrder: external_exports.enum(["asc", "desc"]).optional()
});

// mcp/tools/issues/search.ts
var searchIssuesSchema = external_exports.object({
  query: external_exports.string().min(1).describe("Search query string"),
  fields: external_exports.array(external_exports.enum(["title", "description", "labels"])).optional().describe("Fields to search in"),
  filters: IssueFiltersSchema.optional().describe("Additional filters"),
  sortBy: external_exports.enum(["created", "updated", "priority", "status"]).optional().describe("Sort field"),
  sortOrder: external_exports.enum(["asc", "desc"]).optional().describe("Sort direction"),
  limit: external_exports.number().int().positive().max(1e3).optional().describe("Maximum number of results")
});
async function searchIssuesTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    const options2 = {
      query: params.query,
    ...
    const issues = await issueProvider.searchIssues(options2);
...
function formatSearchResults(issues, query) {
  if (issues.length === 0) {
  ...
  issues.forEach((issue) => {
    ...
    const preview = issue.description.length > 150 ? issue.description.substring(0, 147) + "..." : issue.description;
...
// mcp/tools/issues/update.ts
var updateIssueSchema = external_exports.object({
  id: external_exports.string().min(1).describe("Issue ID to update"),
  title: external_exports.string().min(1).max(200).optional().describe("New issue title"),
  description: external_exports.string().min(1).optional().describe("New issue description"),
  type: external_exports.enum(["issue", "specification", "idea"]).optional().describe("Updated artifact type"),
  status: external_exports.enum(["open", "in_progress", "closed", "cancelled"]).optional().describe("New issue status"),
  priority: external_exports.enum(["low", "medium", "high", "critical"]).optional().describe("New issue priority"),
  labels: external_exports.array(external_exports.string()).optional().describe("New labels for the issue"),
  assignee: external_exports.string().optional().describe("New assignee (use empty string to unassign)"),
  project: external_exports.string().optional().describe("New project association (use empty string to clear)"),
  wranglerContext: external_exports.object({
    agentId: external_exports.string().optional(),
    parentTaskId: external_exports.string().optional(),
    estimatedEffort: external_exports.string().optional()
  }).optional().describe("Updated Wrangler context information")
});
async function updateIssueTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    ...
    const request = {
      ...
      ...params.assignee !== void 0 && { assignee: params.assignee || void 0 },
      ...params.project !== void 0 && { project: params.project || null },
      ...params.wranglerContext && { wranglerContext: params.wranglerContext }
    };
...
// mcp/tools/issues/get.ts
var getIssueSchema = external_exports.object({
  id: external_exports.string().min(1).describe("Issue ID to retrieve")
});
async function getIssueTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
...
// mcp/tools/issues/delete.ts
var deleteIssueSchema = external_exports.object({
  id: external_exports.string().min(1).describe("Issue ID to delete"),
  confirm: external_exports.boolean().describe("Confirmation flag to prevent accidental deletion")
});
async function deleteIssueTool(params, providerFactory) {
  try {
    if (!params.confirm) {
      return {
        content: [
...
// mcp/tools/issues/labels.ts
var issueLabelsSchema = external_exports.object({
  operation: external_exports.enum(["list", "add", "remove"]).describe("Operation to perform"),
  issueId: external_exports.string().optional().describe("Issue ID (required for add/remove operations)"),
  labels: external_exports.array(external_exports.string()).optional().describe("Labels to add or remove")
});
async function issueLabelsTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    if (params.operation === "list") {
      if (params.issueId) {
        const issue2 = await issueProvider.getIssue(params.issueId);
        if (!issue2) {
          return {
...
${labels.join(", ") || "No labels found"}`
    ...
    if (params.operation === "add") {
      newLabels = [.../* @__PURE__ */ new Set([...issue.labels, ...params.labels])];
    } else {
      newLabels = issue.labels.filter((l) => !params.labels.includes(l));
    }
...
// mcp/tools/issues/metadata.ts
var issueMetadataSchema = external_exports.object({
  operation: external_exports.enum(["get", "set", "remove"]).describe("Operation to perform"),
  issueId: external_exports.string().min(1).describe("Issue ID"),
  key: external_exports.string().optional().describe("Metadata key (required for set/remove operations)"),
  value: external_exports.any().optional().describe("Metadata value (required for set operation)")
});
async function issueMetadataTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    const issue = await issueProvider.getIssue(params.issueId);
    if (!issue) {
...
\`\`\``
    ...
    if (params.operation === "set") {
      ...
      delete newContext[params.key];
      await issueProvider.updateIssue({
        id: params.issueId,
        wranglerContext: Object.keys(newContext).length > 0 ? newContext : void 0
      });
      return {
        content: [
          {
...
// mcp/tools/issues/projects.ts
var issueProjectsSchema = external_exports.object({
  operation: external_exports.enum(["list", "add", "remove"]).describe("Operation to perform"),
  issueId: external_exports.string().optional().describe("Issue ID (required for add/remove operations)"),
  project: external_exports.string().optional().describe("Project name for add operations")
});
async function issueProjectsTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    if (params.operation === "list") {
...
// mcp/providers/markdown.ts
var path2 = __toESM(require("path"), 1);
var fsExtra = __toESM(require_lib(), 1);

// mcp/providers/base.ts
...
function loadWorkspaceSchema(basePath) {
...
function getMCPDirectories(basePath) {
...
var DEFAULT_IDEA_DIR = schemaDefaults.ideasDirectory;
var MarkdownIssueProvider = class extends IssueProvider {
  basePath;
  settings;
  issueCounter = 0;
  collectionDirs = /* @__PURE__ */ new Map();
  constructor(settings, _config) {
    super();
    this.settings = settings;
    this.basePath = path2.resolve(settings.basePath || process.cwd());
  ...
  async updateIssue(request) {
    ...
    const updatedIssue = {
      ...
      project: request.project !== void 0 ? request.project ?? void 0 : existingIssue.project,
  ...
  async listIssues(filters) {
    ...
    return issues.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime()).slice(offset, offset + limit);
  }
  async searchIssues(options2) {
    const allIssues = await this.listIssues(options2.filters);
    const query = options2.query.toLowerCase();
    const fields = options2.fields || ["title", "description", "labels"];
    ...
    if (options2.sortBy) {
      matchingIssues.sort((a, b) => {
        ...
        return options2.sortOrder === "desc" ? -result : result;
      });
    }
  ...
  registerCollection(type, directory) {
    ...
    const absolute = path2.isAbsolute(directory) ? directory : path2.join(this.basePath, directory);
  ...
  getCollections(types) {
    ...
    return entries.filter(([type]) => set.has(type)).map(([type, directory]) => ({ type, directory }));
  }
  parseIssueFromFile(parsed, filePath, defaultType) {
    const data = parsed.data;
    const type = data.type || defaultType || "issue";
    return {
      id: data.id || path2.basename(filePath, ".md"),
      ...
      closedAt: data.closedAt ? new Date(data.closedAt) : void 0,
  ...
  slugify(text) {
    let slug = text.toLowerCase().replace(/[^\w\s-]/g, "").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "").substring(0, 50);
  ...
  assertWithinWorkspace(targetPath, action) {
    ...
    const relative2 = path2.relative(this.basePath, resolvedTarget);
...
// mcp/types/errors.ts
function createErrorResponse(code, message, options2 = {}) {
  const errorDetails = {
    code,
    message,
    ...
    remediation: options2.remediation
  };
  if (options2.includeStack && options2.error) {
    errorDetails.stack = options2.error.stack;
  }
  let text = `Error [` + code + `]: ` + message;
  if (options2.remediation) {
    text += `

Suggested fix: ` + options2.remediation;
  }
  if (options2.details) {
    text += `

Details: ` + JSON.stringify(options2.details, null, 2);
  }
  return {
...
var ERROR_REMEDIATIONS = {
  ["TOOL_NOT_FOUND" /* TOOL_NOT_FOUND */]: "Check that the tool name is correct and the server is properly configured.",
  ["VALIDATION_ERROR" /* VALIDATION_ERROR */]: "Review the tool parameters and ensure they match the expected schema.",
  ["PERMISSION_DENIED" /* PERMISSION_DENIED */]: "Verify you have the necessary permissions to perform this operation.",
  ["RATE_LIMIT_EXCEEDED" /* RATE_LIMIT_EXCEEDED */]: "Wait a moment before retrying the operation.",
  ["RESOURCE_NOT_FOUND" /* RESOURCE_NOT_FOUND */]: "Verify the resource ID or path is correct.",
  ["FILE_TOO_LARGE" /* FILE_TOO_LARGE */]: "Reduce the file size or increase the configured limit.",
  ["PATH_TRAVERSAL_DENIED" /* PATH_TRAVERSAL_DENIED */]: "Use relative paths within the workspace directory.",
  ["ISSUE_NOT_FOUND" /* ISSUE_NOT_FOUND */]: "Verify the issue ID exists. Use issues_list to see available issues.",
  ["CONFIGURATION_ERROR" /* CONFIGURATION_ERROR */]: "Check your workspace configuration and ensure all required fields are set."
};
function getRemediation(code) {
  return ERROR_REMEDIATIONS[code];
}

// mcp/tools/issues/all-complete.ts
var issueArtifactTypeSchema = external_exports.enum(["issue", "specification", "idea"]);
var issuesAllCompleteSchema = external_exports.object({
  issueIds: external_exports.array(external_exports.string().min(1)).min(1).optional().describe("Restrict check to specific issue IDs"),
  parentTaskId: external_exports.string().optional().describe("Filter issues whose wranglerContext.parentTaskId matches"),
  labels: external_exports.array(external_exports.string()).optional().describe("Filter by labels (any match)"),
  project: external_exports.string().optional().describe("Filter by project identifier"),
  types: external_exports.array(issueArtifactTypeSchema).optional().describe("Restrict to artifact types")
}).describe("Check if scoped Wrangler issues are complete.");
var COMPLETED_STATUSES = /* @__PURE__ */ new Set(["closed", "cancelled"]);
async function issuesAllCompleteTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    const filters = {
      ids: params.issueIds,
      ...
      project: params.project,
      types: params.types,
      type: params.types ? void 0 : "issue",
      limit: 1e3,
      offset: 0
    };
    const issues = await issueProvider.listIssues(filters);
...
function buildCompletionSummary(issues) {
  ...
  if (noIssues) {
    ...
    const pendingList = pendingIssues.length > 0 ? ` Pending: ${pendingIssues.map((issue) => `${issue.id} (${issue.status})`).join(", ")}.` : "";
...
// mcp/tools/issues/mark-complete.ts
var markCompleteSchema = external_exports.object({
  id: external_exports.string().min(1).describe("Issue ID to mark as closed"),
  note: external_exports.string().optional().describe("Optional completion note to append to the description")
});
async function markCompleteIssueTool(params, providerFactory) {
  try {
    const issueProvider = providerFactory.getIssueProvider();
    const issue = await issueProvider.getIssue(params.id);
    if (!issue) {
...
var WranglerMCPServer = class {
  ...
  setupTools() {
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : "Unknown error";
        let errorCode = "TOOL_EXECUTION_ERROR" /* TOOL_EXECUTION_ERROR */;
        if (error instanceof external_exports.ZodError) {
          ...
          errorCode = "PATH_TRAVERSAL_DENIED" /* PATH_TRAVERSAL_DENIED */;
        }
        const errorResponse = createErrorResponse(
          ...
          `Error executing tool ${name}: ${message}`,
          {
            ...
            details: error instanceof external_exports.ZodError ? error.errors : void 0
          }
        );
        return {
  ...
  getAvailableTools() {
    return [
      {
        name: "issues_create",
        description: "Create a new Wrangler issue (Markdown under .wrangler/issues) with optional status, priority, assignee, and labels for local project tracking.",
        inputSchema: zodToJsonSchema(createIssueSchema)
      },
      {
        name: "issues_list",
        description: "List Wrangler issues in the workspace with filters for status, priority, labels, assignee, or project to review current local work items.",
        inputSchema: zodToJsonSchema(listIssuesSchema)
      },
      {
        name: "issues_search",
        description: "Search across Wrangler issue titles, descriptions, and labels to locate tasks by keyword while honoring the same filtering options as issues_list.",
        inputSchema: zodToJsonSchema(searchIssuesSchema)
      },
      {
        name: "issues_update",
        description: "Modify an existing Wrangler issue's frontmatter (status, priority, labels, assignee, etc.) after work progresses or context changes.",
        inputSchema: zodToJsonSchema(updateIssueSchema)
      },
      {
        name: "issues_get",
        description: "Retrieve the full Markdown content and metadata for a single Wrangler issue when you need complete local context before taking action.",
        inputSchema: zodToJsonSchema(getIssueSchema)
      },
      {
        name: "issues_delete",
        description: "Remove a Wrangler issue file; requires `confirm: true` and should be used only when local issue history can be safely discarded.",
        inputSchema: zodToJsonSchema(deleteIssueSchema)
      },
      {
        name: "issues_labels",
        description: "List, add, or remove labels across Wrangler issues to keep local tagging consistent with project conventions.",
        inputSchema: zodToJsonSchema(issueLabelsSchema)
      },
      {
        name: "issues_metadata",
        description: "Read or update the Wrangler-specific metadata block (`wranglerContext`) on an issue for workflow IDs, parent tasks, or other structured context.",
        inputSchema: zodToJsonSchema(issueMetadataSchema)
      },
      {
        name: "issues_projects",
        description: "List Wrangler issue projects or assign/clear a project on a local issue to keep work grouped by initiative.",
        inputSchema: zodToJsonSchema(issueProjectsSchema)
      },
      {
        name: "issues_all_complete",
        description: "Check whether all Wrangler issues are complete and receive a summary of pending vs. finished work.",
