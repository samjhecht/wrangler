...
export async function sessionGetTool(params, storageProvider) {
    try {
        let session;
        if (params.sessionId) {
        ...
        return createErrorResponse(MCPErrorCode.TOOL_EXECUTION_ERROR, `Failed to get session: ${message}`, { details: { sessionId: params.sessionId } });
    }
}
